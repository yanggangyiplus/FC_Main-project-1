...
- 이미지 태그 수정 ❌
- alt 텍스트 수정 ❌
- **h2 태그 내용 변경 ❌** (반드시 "서론", "본론", "결론" 그대로 유지!)
- **h2 태그 제거 ❌** (구조 마커로 사용되므로 절대 삭제 금지!)

👉 **오직 문체, 말투, 리듬만 조정**
👉 **h1, h2 태그는 절대 건드리지 말 것!**
...
2. HTML 구조 유지 (h1, h2, p, img 태그)
3. **h2 태그는 반드시 원본 그대로 유지!**
   - 입력에 <h2>서론</h2>이면 → 출력도 <h2>서론</h2>
   - 입력에 <h2>본론</h2>이면 → 출력도 <h2>본론</h2>
   - 입력에 <h2>결론</h2>이면 → 출력도 <h2>결론</h2>
4. 이미지 태그 원본 그대로 유지
5. p 태그 안의 텍스트만 자연스럽게 수정
...
⚠️ 주의: h2 태그 내용을 절대 바꾸거나 제거하지 말 것!
