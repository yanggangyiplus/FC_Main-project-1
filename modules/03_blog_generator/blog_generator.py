...
📌 제목 작성 규칙 (SEO 최적화 필수!)
━━━━━━━━━━━━━━━━━━
⚠️ 매우 중요: 제목은 반드시 한글로 작성!
⚠️ 영어 제목 절대 금지! (예: "Latest IT Technology Trends" ❌)

🎯 SEO 핵심 규칙:
1. 기사에 등장하는 **핵심 고유명사**(기업명, 인물명, 제품명)를 제목에 반드시 포함
2. 이슈의 **핵심 키워드**를 제목에 포함
3. 검색했을 때 바로 노출될 수 있는 구체적 제목

✅ 좋은 예시 (핵심 키워드 포함):
- "KT 개인정보 유출 사태, 피해 규모와 대응 방법 정리"
- "쿠팡 대규모 화재 사고, 책임 공방과 정부 대응 총정리"
- "삼성전자 반도체 적자, 원인과 전망 분석"
- "네이버 AI 서비스 업데이트, 달라진 점 핵심 정리"

❌ 나쁜 예시:
- "요즘 IT 이슈 정리해봤어요" (너무 모호함)
- "Latest Technology Trends" (영어 금지)
- "최신 기술 동향" (구체적 키워드 없음)

제목 작성 프로세스:
1. 기사들에서 가장 많이 언급되는 기업/인물/제품명 파악
2. 해당 고유명사 + 이슈 키워드로 제목 구성
3. 검색 시 노출될 수 있도록 구체적으로 작성
...
1. **핵심 고유명사 추출**: 기사들에서 가장 많이 언급되는 기업명/인물명/제품명 파악
2. **제목에 포함**: 핵심 고유명사를 제목에 반드시 포함
3. **본문에 반복**: 핵심 키워드를 본문 전체에 5~8회 자연스럽게 분산 배치
   - 서론: 1~2회
   - 본론: 3~4회
   - 결론: 1~2회

예시 (KT 개인정보 유출 기사의 경우):
- 제목: "KT 개인정보 유출 사태, 피해 규모와 대응 방법 정리"
- 본문에 "KT", "개인정보 유출", "KT 고객" 등 키워드 반복
...
6. **마커는 독립된 줄에 배치하고 <p> 태그로 감싸지 말 것!** ###DIVIDER1### ✅  <p>###DIVIDER1###</p> ❌
7. 마커 안에 다른 텍스트 넣지 말 것! ###DIVIDER1### ✅  <p>구분선 ###DIVIDER1###</p> ❌
...
{html}
    ...
    def extract_image_placeholders(self, html: str) -> list:
        ...
        if marker_matches:
            ...
            logger.info(f"추출된 플레이스홀더: {len(placeholders)}개 (###IMG### 마커)")
            return placeholders
        
        # 2. 레거시: <img src="PLACEHOLDER"> 형식
        legacy_pattern = r'<img[^>]*src="PLACEHOLDER"[^>]*alt="([^"]*)"[^>]*>'
        legacy_matches = re.finditer(legacy_pattern, html)
        for i, match in enumerate(legacy_matches):
            alt_text = match.group(1)
            placeholders.append({
                "index": i,
                "alt": alt_text,
                "marker": 'src="PLACEHOLDER"',
                "tag": match.group(0)
            })
